...
def login_using_env(self, env_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "message": "dotenv is not installed; please install python-dotenv",
        }
    ...
    if not env_path.exists():
        return {"success": False, "message": f"Env file not found at {env_path}"}

    load_dotenv(env_path)

    username = os.getenv("LANIS_API_USERNAME")
    password = os.getenv("LANIS_API_PASSWORD")
    school_id = os.getenv("LANIS_API_SCHOOL_ID")

    if not username or not password or not school_id:
        missing = [
            key
            for key, value in [
                ("LANIS_API_USERNAME", username),
                ("LANIS_API_PASSWORD", password),
                ("LANIS_API_SCHOOL_ID", school_id),
            ]
            if not value
        ]
        return {
...
def login(self, school_id: str, username: str, password: str) -> Dict[str, Any]:
    ...
    try:
        ...
        self.session.cookies.set(
            "schulportal_lastschool", school_id, domain="hessen.de"
        )
        self.session.cookies.set(
            "schulportal_logindomain",
        ...
        if "sid" in cookies_dict or "SPH-Session" in cookies_dict:
            ...
            return {
                ...
                "school_id": school_id,
                "encryption_ready": self.cryptor is not None
                and self.cryptor.authenticated,
            }
        else:
            return {
                "success": False,
