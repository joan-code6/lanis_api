    ...
    try:
        ...
        for row in soup.find_all('tr', {'data-book': True}):
            ...
            if teacher_button:
                ...
                if teacher_dropdown:
                    message_link = teacher_dropdown.find('a', title='Nachricht schreiben')
                    if message_link:
        ...
        return {
            'success': False,
            'error': f'Failed to fetch mein Unterricht: {str(e)}'
        }
...
def meinunterricht_get_course(self, course_id: str) -> Dict[str, Any]:
    ...
    if not self.cryptor or not self.cryptor.authenticated:
        return {'success': False, 'error': 'Encryption not initialized for attendance decryption'}
...
def meinunterricht_get_entry_details(self, url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'success': False,
            'error': f'Failed to fetch entry details: {str(e)}'
        }
...
def meinunterricht_get_weekly_view(self) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'success': False,
            'error': f'Failed to fetch weekly view: {str(e)}'
        }
